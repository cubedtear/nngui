/*
 *  PicoGUI is a minimalistic port for https://github.com/wjakob/nanogui
    picogui/picogui.h -- Pull in *everything* from PicoGUI
...
#ifdef _MSC_VER
#define PICOGUI_SNPRINTF _snprintf
#else
#define PICOGUI_SNPRINTF snprintf
#endif
 ...
 *
 * PICOGUI uses this to convert the icon character codes
 * defined in entypo.h
 */
...
#define nvgImageIcon(ctx, name) picogui::__picogui_get_image(ctx, #name, name##_png, name##_png_size)
/// Helper function used by nvgImageIcon
extern int __picogui_get_image(NVGcontext *ctx, const std::string &name, uint8_t *data, uint32_t size);
 ...
 *
 * \ref Widget is the base class of all widgets in \c picogui. It can
...
public:
    struct Anchor {
        ...
        operator std::string() const {
            char buf[50];
            PICOGUI_SNPRINTF(buf, 50, "Format[pos=(%i, %i), size=(%i, %i), align=(%i, %i)]",
...
public:
    ...
    void setValue(Scalar value) {
        char buffer[50];
        PICOGUI_SNPRINTF(buffer, 50, mNumberFormat.c_str(), value);
